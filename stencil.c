...
//avx2+parallel
/*static void nvx_stencil_func(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings) {
    const int margin_x = (STENCIL_WIDTH - 1) / 2;
    const int margin_y = (STENCIL_HEIGHT - 1) / 2;

    // Chargement des coefficients du motif stencil en utilisant AVX2
    __m256 reg_stencil_coef = _mm256_set1_ps(stencil_coefs[1 * STENCIL_WIDTH + 1]);

    ELEMENT_TYPE *p_temporary_mesh = _mm_malloc(p_settings->mesh_width * p_settings->mesh_height * sizeof(*p_mesh), 32);

    
    // Itération sur les éléments de la grille
    for (int x = margin_x; x < p_settings->mesh_width - margin_x; x++) {
        for (int y = margin_y; y < p_settings->mesh_height - margin_y; y++) {
            __m256 reg_value = _mm256_loadu_ps(&p_mesh[y * p_settings->mesh_width + x]);

            // Itération sur les éléments du motif stencil
            for (int stencil_x = 0; stencil_x < STENCIL_WIDTH; stencil_x++) {
                for (int stencil_y = 0; stencil_y < STENCIL_HEIGHT; stencil_y++) {
                    int neighbor_index = (y + stencil_y - margin_y) * p_settings->mesh_width + (x + stencil_x - margin_x);
                    __m256 reg_neighbor_value = _mm256_loadu_ps(&p_mesh[neighbor_index]);

                    // Multiplication des valeurs du voisin par les coefficients du motif et ajout à la valeur actuelle
                    reg_value = _mm256_add_ps(reg_value, _mm256_mul_ps(reg_neighbor_value, reg_stencil_coef));
                }
            }

            // Stockage de la valeur calculée dans la grille temporaire
            _mm256_storeu_ps(&p_temporary_mesh[y * p_settings->mesh_width + x], reg_value);
        }
    }
     
   
    // Copie des données de la grille temporaire vers la grille d'origine en utilisant AVX2
    for (int x = margin_x; x < p_settings->mesh_width - margin_x; x++) {
        for (int y = margin_y; y < p_settings->mesh_height - margin_y; y++) {
            __m256 reg_temp_mesh = _mm256_loadu_ps(&p_temporary_mesh[y * p_settings->mesh_width + x]);
            _mm256_storeu_ps(&p_mesh[y * p_settings->mesh_width + x], reg_temp_mesh);
        }
    }
...
//Vesrion simplifiée: Simplification des boucles for
static void naive_stencil_func1(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        const int margin_x = (STENCIL_WIDTH - 1) / 2;
        const int margin_y = (STENCIL_HEIGHT - 1) / 2;
...
//Version AVX2 après simplification des boucles for
...
static void run(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        ...
        {
                naive_stencil_func1(p_mesh, p_settings);
