...
#define DEFAULT_MESH_WIDTH 4000 //10
#define DEFAULT_MESH_HEIGHT 1000 //7
...
//Vesrion simplifiée: Simplification des boucles for(Changer les deux boucles for x et for y par seuelement for Ig)
...
/*Dans cette version, il faut que la condition (end-beg) % (REG_NB_ELEMENTS) == 0 soit vérifié*/
...
__attribute__((noinline)) void simd_avx2(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        ...
        assert( (end-beg) % (REG_NB_ELEMENTS) == 0);
...
//Définir le REG_BYTES
#define REG_BYTES (sizeof(__m256))
//Définir le nombre d'éléments par registre
#define REG_NB_ELEMENTS (REG_BYTES / sizeof(float))

//La bonne version avx2 
...
static void run(ELEMENT_TYPE *p_mesh, struct s_settings *p_settings)
{
        ...
        for (i = 0; i < p_settings->nb_iterations; i++)
        {
                ...
                //naive_stencil_func1(p_mesh, p_settings); // Simplification de la boucle for 
                //avx2(p_mesh, p_settings); //marche que lorsuqe le condition (end-beg) % REG_NB_ELEMENTS est vérifée
                func_simd_avx2(p_mesh, p_settings); //La bonne version pour avx2
                if (p_settings->enable_output)
                {
                        char filename[32];
...
static int check(const ELEMENT_TYPE *p_mesh, ELEMENT_TYPE *p_mesh_copy, struct s_settings *p_settings)
{
        ...
        for (i = 0; i < p_settings->nb_iterations; i++)
        {
                naive_stencil_func1(p_mesh_copy, p_settings);
