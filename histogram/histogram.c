...
/*L'histogramme divise la plage de valeurs en plusieurs bins (plages) et compte 
le nombre d'éléments du tableau qui tombent dans chaque bin */
...
//Version OpenMP
static void mp_compute_histogram(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings)
{
        //Intialiser l'histogramme à zéro
        memset(histogram, 0, p_settings->nb_bins * sizeof(*histogram));
