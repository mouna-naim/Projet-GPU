...
#define DEFAULT_ARRAY_LEN 17 //10
#define DEFAULT_NB_BINS 19
...
//Definir le kernel
__global__ void kernel_histogram(ELEMENT_TYPE *array, int *histogram, ELEMENT_TYPE array_len, ELEMENT_TYPE scale, ELEMENT_TYPE lower_bound, int nb_bins) 

{
    ...
    if (i < array_len) {
        ...
        //la boucle qui calcule bound a été simplifiée après un petit calcul fait
        int cond = (int)((value - lower_bound - (scale / 2)) * nb_bins);
        if (cond >= 0 && cond < scale) {
            atomicAdd(&histogram[cond], 1);
        }
    }
}


void cuda_compute_histogram(const ELEMENT_TYPE *array, int *histogram, struct s_settings *p_settings) {

    const ELEMENT_TYPE offset = p_settings->lower_bound;
    const ELEMENT_TYPE scale = p_settings->upper_bound - p_settings->lower_bound;
    
    ELEMENT_TYPE *gpu_array;
    int *gpu_histogram;

    // Allocation de la mémoire sur le GPU
    cudaMalloc(&gpu_array, p_settings->array_len * sizeof(ELEMENT_TYPE));
    cudaError_t err;
    ...
    cudaMalloc(&gpu_histogram, p_settings->nb_bins * sizeof(int));

    cudaMemset(gpu_histogram, 0, p_settings->nb_bins * sizeof(int));

    // Copie des données CPU vers GPU
    cudaMemcpy(gpu_array, array, p_settings->array_len * sizeof(ELEMENT_TYPE), cudaMemcpyHostToDevice);

    int THREADS = 256;
    int BLOCKS = (p_settings->array_len + THREADS - 1) / THREADS;

    // Appel du kernel
    kernel_histogram<<<BLOCKS, THREADS>>>(gpu_array, gpu_histogram, p_settings->array_len, scale, offset, p_settings->nb_bins);


   // Copie des données GPU vers CPU
    ...
    if (p_settings->enable_output)
        {
                ...
                write_histogram_to_file(file, histogram, p_settings);
                fclose(file);
        }
        ...
        if (p_settings->enable_verbose)
        {
                ...
                print_histogram(histogram, p_settings);
                printf("\n\n");
        }
